...
fn part2(input: []const u8) !usize {
    ...
    const dist_slice = try distances.toOwnedSlice();
    defer allocator.free(dist_slice);
    const time = try std.fmt.parseInt(usize, time_slice, 10);
    const dist = try std.fmt.parseInt(usize, dist_slice, 10);
    var ways: usize = 0;
    for (1..time) |option| {
        const traveled = (option * (time - option));
        if (traveled > dist) {
...
test "input one" {
    try std.testing.expectEqual(part1(@embedFile("day06.txt")), 800_280);
}
...
test "input two" {
    try std.testing.expectEqual(part2(@embedFile("day06.txt")), 45_128_024);
}
